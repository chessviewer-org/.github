...
- [`.github/ISSUE_TEMPLATE/`](./.github/ISSUE_TEMPLATE) and
  ...
  fallback issue and PR templates
- [`FUNDING.yml`](./FUNDING.yml) — the org-wide Sponsor button

Any repository that ships its own version of these files uses that instead;
otherwise it inherits the defaults here. There's nothing to install or run —
GitHub picks these up automatically.
...
docs, or tell other people about it.

## License

Unless a repository states otherwise, projects are released under the license in
their own repository. See [`LICENSE`](./LICENSE) for the license that applies to
